...
public class DesignItemDaoImpl implements DesignItemDao {
	...
	public void update(DesignItem di2) {
		// TODO Auto-generated method stub
		Session session = sessionFactory.getCurrentSession();
		String hql = "update DesignItem di set di.mProductId=?, di.titleItem=?, di.requestDueDate=?, di.note=?, di.updatedDate=? where di.id=?";
		Query query = session.createQuery(hql);
		...
		query.setParameter(4, di2.getUpdatedDate());
		query.setParameter(5, di2.getId());
